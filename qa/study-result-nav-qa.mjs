...
async function runNavSequence(viewport, theme) {
  ...
  check(`${tag}: Andorra — Quiz CTA still present`, await card().getByRole('button', { name: /^quiz$/i }).isVisible())
...
async function runCardHeight(viewport, theme) {
  ...
  const quiz = p.getByRole('button', { name: /^quiz$/i })
  const backToStudy = p.getByRole('button', { name: /^back to study$/i })
  await quiz.scrollIntoViewIfNeeded()
  check(`${tag}: Quiz CTA comfortably inside the card`, await quiz.isVisible())
