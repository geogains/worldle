...
export interface ResultModalProps {
  ...
  /** Primary/secondary actions rendered under the result. */
  actions: ReactNode
}
...
export function ResultModal(props: ResultModalProps) {
  const { open, onClose, title, mode, answer, guesses, won, shareLabel, sharePuzzleNumber, actions } = props
  const headline = won ? (WIN_HEADLINES[guesses.length - 1] ?? 'Well done') : 'Not this time'
  return (
    <Modal open={open} onClose={onClose} title={title}>
      <p className="text-center text-[1.6rem] font-extrabold tracking-[-0.01em]">{headline}</p>
